...
void getDirection(_S_BULLET_OBJECT *pBullet,_S_Plane *pPlane,double *vx,double *vy)
{			
	...
	while(bLoop) {
		...
		if(kbhit() != 0) {
			...
			if(ch == 'q') {
				...
				getDirection(&gTestBullet,&gPlayerObject);
				gTestBullet.pfFire(&gTestBullet,
				gTestBullet.m_fXpos,gTestBullet.m_fYpos.
				10.0,vx,vy,10);
